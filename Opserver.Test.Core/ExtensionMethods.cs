namespace StackExchange.Opserver.Helpers
{
    ...
    public static partial class ExtensionMethods
    {
        ...
        {
            if (string.IsNullOrEmpty(path) || lineNumber == 0) return sql;
            ...
            unchecked
            {
                ...
                if (!string.IsNullOrEmpty(comment)) key = key * 23 + comment.GetHashCode();
            }
            ...
            var ret = sql.Substring(0, i) + " /* " + path.Substring(split) + "@" + lineNumber.ToString() + (!string.IsNullOrEmpty(comment) ? " - " + comment : "") + " */" + commentWrap + sql.Substring(i);
